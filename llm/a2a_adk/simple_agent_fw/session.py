...
from events import SAFEvent # EventActions might not be directly used here, SAFEvent is.
...
class Session(ABC):
    ...
    @abstractmethod
    def add_event(self, event: SAFEvent) -> None:
        """Adds an event to the session's history."""
        pass

    @abstractmethod
    def get_events(self) -> List[SAFEvent]:
    ...
    @abstractmethod
    def get_state_value(self, key: str, default: Optional[Any] = None) -> Optional[Any]:
...
class InMemorySession(Session):
    ...
    def __init__(self, session_id: str, initial_state: Optional[Dict[str, Any]] = None):
        ...
        self._state: Dict[str, Any] = dict(initial_state) if initial_state is not None else {}
    ...
    def get_events_by_author(self, author: str) -> List[SAFEvent]:
        """Returns all events authored by a specific author."""
        return [event for event in self.get_events() if event.author == author]

    def get_events_with_actions(self) -> List[SAFEvent]:
        """Returns all events that have associated actions."""
        return [event for event in self.get_events() if event.actions is not None]

    def get_events_with_state_updates(self) -> List[SAFEvent]:
    ...
    def get_last_n_events(self, n: int) -> List[SAFEvent]:
        """Returns the last N events."""
        if (n <= 0):
            return []
        return self._events[-n:] if n <= len(self._events) else self._events

    def get_last_user_event(self) -> Optional[SAFEvent]:
        """Returns the most recent event authored by 'user' (or common user identifiers)."""
        user_author = "user"  
        for event in reversed(self.get_events()):
            if event.author == user_author:
                return event
        return None

    def get_last_agent_event(self, agent_author: str = "agent") -> Optional[SAFEvent]:
